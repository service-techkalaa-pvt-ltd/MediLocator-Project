"""
Load pharmacy data into SQLite database
Run: python load_pharmacy_data.py
"""
import os
import django
import random
...
def load_pharmacies():
    """Load all pharmacy data including real and dummy records"""
    ...
    # Real pharmacy data from your dataset
    real_pharmacies = [
    ...
    # Generate 150+ dummy pharmacies across Indian cities
    dummy_pharmacies = generate_dummy_pharmacies()

    # All pharmacies to insert
    all_pharmacies = real_pharmacies + dummy_pharmacies
        ...
        try:
            pharmacy, created = Pharmacy.objects.get_or_create(
                ...
                defaults={
                    ...
                    'phone': pharmacy_data.get('phone', '9876543210'),
                    'owner': admin_user,
                    'verified': True,
            ...
            skipped_count += 1
            print(f"✗ Error creating {pharmacy_data['name']}: {str(e)}")

    print(f"\n" + "="*60)
    print(f"✓ Successfully created: {created_count} pharmacies")
    print(f"⊘ Skipped (already exists): {skipped_count} pharmacies")
    print(f"📊 Total pharmacies in database: {Pharmacy.objects.count()}")
    print(f"="*60 + "\n")
...
def generate_dummy_pharmacies():
    ...
    for city, base_lat, base_lon in cities:
        ...
        for i in range(8):
            ...
            street_names = ['Main Street', 'Market Road', 'Central Plaza', 'High Street', 
                          'New Road', 'Commerce Street', 'Station Road', 'Civil Lines']
            street = random.choice(street_names)
            
            dummy_list.append({
                'name': name,
                'address': f"Shop {i+1}, {street}, {city}, Maharashtra, India",
                'city': city,
                'rating': rating,
                'lat': lat,
                'lon': lon,
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("  📦 Loading Pharmacy Data into SQLite Database")
    print("="*60 + "\n")
    load_pharmacies()
